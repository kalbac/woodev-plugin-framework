...
- [Array short syntax](#array-short-syntax)
- [PHP 7.4+ Features](#php-74-features)
- [Linting](#linting)
...
```php
call_user_func_array( [ $obj, 'method' ], [ $code ] );
call_user_func_array( [ $obj, 'process' ], [ $id, $status, $data ] );
```

**Wrong:**

```php
call_user_func_array( [ $obj, 'method' ], [ 'country_code' => $code ] );
call_user_func_array( [ $obj, 'process' ], [ 'order_id' => $id, 'status' => $status ] );
```

## Array short syntax

Use array short syntax whenever possible.

## PHP 7.4+ Features

Use the features of PHP 7.4+ as much as possible and appropriately.

**Good:**

```php
fn( $data ) => $this->process( $data );
return $cache ??= $this->get_cache();
```

**Avoid:**

```php
function( $data ) { return $this->process( $data ); }
if( ! $cache ) { $cache = $this->get_cache(); } return $cache;
```
