...
if ( ! class_exists( 'Woodev_Hook_Deprecator' ) ) :

	/**
	 * Woodev Hook Deprecator Class
	 *
	 * This class handles triggering PHP notices for deprecated and removed hooks
	 */
	class Woodev_Hook_Deprecator {
		 ...
		 *
		 * @param mixed ...$args all hook arguments passed by WordPress
		 * @return mixed
		 */
		public function map_deprecated_hook( ...$args ) {
		...
		public function trigger_deprecated_errors() {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG && apply_filters( 'woodev_plugin_framework_show_deprecated_hook_notices', true ) ) {
		...
		protected function trigger_error( $old_hook_name, $hook ) {
			...
			// e.g. WooCommerce CDEK: "wc_edostavka_some_hook" was deprecated in version 1.2.3.
			$message = sprintf(
				'%1$s: action/filter "%2$s" was %3$s in version %4$s. ',
				$this->plugin_name,
				$old_hook_name,
			...
			// e.g. Use "wc_edostavka_some_new_hook" instead.
			...
			Woodev_Helper::trigger_error( $message );
		}
	}
