...
if ( ! class_exists( 'Woodev_Lifecycle' ) ) :
	...
	class Woodev_Lifecycle {
		...
		public function init() {
			...
			if ( version_compare( $installed_version, $plugin_version, '<' ) ) {
				...
				if ( ! $installed_version ) {
					 ...
					 */
					do_action( 'woodev_' . $this->get_plugin()->get_id() . '_installed' );
					 ...
					 */
					do_action( 'woodev_' . $this->get_plugin()->get_id() . '_updated', $installed_version );
				}
		...
		public function handle_activation() {
			...
			if ( ! get_option( 'woodev_' . $this->get_plugin()->get_id() . '_is_active', false ) ) {
				 ...
				 */
				do_action( 'woodev_' . $this->get_plugin()->get_id() . '_activated' );

				update_option( 'woodev_' . $this->get_plugin()->get_id() . '_is_active', 'yes' );
			}
		}
		...
		public function handle_deactivation() {
			...
			if ( Woodev_Plugin_Compatibility::is_enhanced_admin_available() ) {

				Woodev_Notes_Helper::delete_notes_with_source( $this->get_plugin()->get_id_dasherized() );

				// if this is a gateway plugin, also delete the plugin's individual gateway notes
				if ( $this->get_plugin() instanceof Woodev_Payment_Gateway_Plugin ) {

					foreach ( $this->get_plugin()->get_gateways() as $gateway ) {
						Woodev_Notes_Helper::delete_notes_with_source( $gateway->get_id_dasherized() );
					}
				}
			}
			...
			wp_clear_scheduled_hook( 'woodev_weekly_scheduled_events' );

			/**
			 * Fires when the plugin is deactivated.
			 */
			do_action( 'woodev_' . $this->get_plugin()->get_id() . '_deactivated' );

			delete_option( 'woodev_' . $this->get_plugin()->get_id() . '_is_active' );
		}
		...
		public function add_admin_notices() : void {
			...
			foreach ( $this->get_milestone_messages() as $id => $message ) {
				 ...
				 */
				$message = apply_filters( 'woodev_' . $this->get_plugin()->get_id() . '_milestone_message', $this->generate_milestone_notice_message( $message ), $id );
		 ...
		 * @return bool
		 */
		public function trigger_milestone( string $id, string $message, string $since = '1.0.0' ): bool {
		 ...
		 * @return string
		 */
		protected function generate_milestone_notice_message( string $custom_message ): string {
		...
		public function register_milestone_message( string $id, string $message ): bool {
			...
			return update_option( 'woodev_' . $this->get_plugin()->get_id() . '_milestone_messages', $milestone_messages );
		}
		 ...
		 * @return string
		 */
		protected function get_event_history_option_name(): string {
			return 'woodev_' . $this->get_plugin()->get_id() . '_lifecycle_events';
		}
		 ...
		 * @return bool
		 */
		public function set_milestone_version( string $version ): bool {

			$this->milestone_version = $version;

			return update_option( 'woodev_' . $this->get_plugin()->get_id() . '_milestone_version', $version );
		}
		 ...
		 * @return string
		 */
		public function get_milestone_version(): string {

			if ( ! $this->milestone_version ) {
				$this->milestone_version = get_option( 'woodev_' . $this->get_plugin()->get_id() . '_milestone_version', '' );
			}
		 ...
		 */
		protected function set_installed_version( string $version ) {
