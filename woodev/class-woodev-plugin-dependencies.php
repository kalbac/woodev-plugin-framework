...
if ( ! class_exists( 'Woodev_Plugin_Dependencies' ) ) :
...
class Woodev_Plugin_Dependencies {
	...
	public function add_php_extension_notices() {
		...
		if ( count( $missing_extensions ) > 0 ) {
			...
			$this->add_admin_notice( 'woodev-' . $this->get_plugin()->get_id_dasherized() . '-missing-extensions', $message, 'error' );
		}
	}
	...
	public function add_php_function_notices() {
		...
		if ( count( $missing_functions ) > 0 ) {
			...
			$this->add_admin_notice( 'woodev-' . $this->get_plugin()->get_id_dasherized() . '-missing-functions', $message, 'error' );
		}
	}
	...
	public function add_php_settings_notices() {
		...
		if ( isset( $_GET['page'] ) && 'wc-settings' === $_GET['page'] ) {
			...
			if ( count( $bad_settings ) > 0 ) {
				...
				foreach ( $bad_settings as $setting => $values ) {
					...
					if ( ! empty( $values['type'] ) && 'min' === $values['type'] ) {
						...
						/** translators: Placeholders: %s - a PHP setting value */
							__( '%s or higher', 'woodev-plugin-framework' ),
							$setting_message
						);
					}
				...
				$message .= __( 'Please contact your hosting provider or server administrator to configure these settings.', 'woodev-plugin-framework' );

				$this->add_admin_notice( 'woodev-' . $this->get_plugin()->get_id_dasherized() . '-incompatibile-php-settings', $message, 'warning' );
			}
		}
	}
		...
		if ( version_compare( PHP_VERSION, '7.4.0', '<' ) ) {
			...
			/* translators: Placeholders: %1$s - <strong>, %2$s - </strong> */
				__( 'Hey there! We\'ve noticed that your server is running %1$san outdated version of PHP%2$s, which is the programming language that WooCommerce and its extensions are built on.
					...
					Your hosting provider can do this for you. %4$sHere are some resources to help you upgrade%5$s and to explain PHP versions further.', 'woodev-plugin-framework' ),
				'<strong>', '</strong>',
				esc_html( $this->get_plugin()->get_plugin_name() ),
				'<a href="https://woodev.ru/support">', '</a>'
			);

			$message .= '</p>';

			$this->add_admin_notice( 'woodev-deprecated-php-version', $message, 'error' );
		}
	}
	...
	public function get_active_scripts_optimization_plugins() {
		 ...
		 */
		$plugins = (array) apply_filters( 'woodev_' . $this->get_plugin()->get_id() . '_scripts_optimization_plugins', [
