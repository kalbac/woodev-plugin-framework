...
if( ! class_exists( 'Woodev_Background_Job_Handler' ) ) :
	 ...
	 *
	 * Subclasses Woodev_Async_Request. Instead of the concept of `batches` used in
	 ...
	 * # Sample usage:
	 *
	 * $background_job_handler = new Woodev_Background_Job_Handler();
	...
	abstract class Woodev_Background_Job_Handler extends Woodev_Async_Request {
		 ...
		 * to create totally different job processing implementations.
		 *
		...
		public function run_debug_tool() {
			...
			if( $this->test_connection() ) {
				...
				$this->debug_message = esc_html__( 'Could not connect. Please ask your hosting company to ensure your server has loopback connections enabled.', 'woodev-plugin-framework' );
				$result              = false;
			}
