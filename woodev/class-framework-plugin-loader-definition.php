...
if ( ! class_exists( Framework_Plugin_Loader_Definition::class, false ) ) :
	...
	class Framework_Plugin_Loader_Definition {
		...
		/**
		 * Creates a loader definition from the temporary legacy register_plugin() shape.
		 *
		 * @since 2.0.0
		 *
		 ...
		 * @return self|null
		 */
		public static function from_legacy_registration(
			...
			$platform = self::PLATFORM_WORDPRESS;

			if ( ! empty( $args['minimum_wc_version'] ) || ! empty( $args['is_payment_gateway'] ) || ! empty( $args['load_shipping_method'] ) ) {
				$platform = self::PLATFORM_WOOCOMMERCE;
			}

			$requirements = [
				'php'       => $args['minimum_php_version'] ?? '0',
				'wordpress' => $args['minimum_wp_version'] ?? '0',
			];

			if ( self::PLATFORM_WOOCOMMERCE === $platform ) {
				$requirements['woocommerce'] = $args['minimum_wc_version'] ?? '0';
			}

			$capabilities = [ self::CAPABILITY_WORDPRESS_PLUGIN ];

			if ( self::PLATFORM_WOOCOMMERCE === $platform ) {
				$capabilities[] = self::CAPABILITY_WOOCOMMERCE_PLUGIN;
			}

			if ( ! empty( $args['is_payment_gateway'] ) ) {
				$capabilities[] = self::CAPABILITY_PAYMENT_GATEWAY;
			}

			if ( ! empty( $args['load_shipping_method'] ) ) {
				$capabilities[] = self::CAPABILITY_SHIPPING_METHOD;
			}

			return self::from_array(
				[
					'plugin_id'         => $args['plugin_id'] ?? self::sanitize_plugin_id( basename( $path, '.php' ) ),
					'plugin_name'       => $plugin_name,
					'plugin_version'    => $args['plugin_version'] ?? '0',
