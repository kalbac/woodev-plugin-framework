...
( function( $ ) {
	 ...
	 * (#274 п.3). Отсутствующий/не-массив список деградирует к `[ 'review' ]` —
	 * поведению фреймворка ДО #274 — а не к пустому списку: разнородный флот, где
	 * это поле пришло от плагина на СТАРОЙ версии фреймворка (без ключа
	 * `pickup_slot_placements` в конфиге), не должен молча остаться вовсе без
	 * триггера.
	 *
	 * @param {Object} entry Запись { store, config }.
	 * @returns {void}
	...
	function placeSlots( entry ) {
		...
		Object.keys( fields ).forEach( function( fieldId ) {
			...
			var placements = Array.isArray( field.pickup_slot_placements ) && field.pickup_slot_placements.length
