 ...
 * весь магазин), поэтому берётся из ПЕРВОГО конфига, где ключ есть. `block_place_order`
 * (issue #725) — тем же способом: выключает A2-гейт целиком, оставляя `#place_order`
 * нетронутым; сервер (`Checkout_Handler::validate()`) при этом не меняется.
 *
 * @file
 * @since 2.0.2
 */
...
( function( $ ) {
	...
	/**
	 * Читает «Блокировать оформление заказа» (issue #725) — стор-синглтон, тем же
	 * способом, что и `field_policy`/`pickup_method_ids`: один `Checkout_Field_Settings`
	 * на весь магазин, поэтому у ВСЕХ конфигов на странице одно и то же значение;
	 * достаточно ПЕРВОГО конфига, где ключ реально присутствует. Отсутствие ключа
	 * (флот со старым PHP, флаг ещё не существовал) — легитимное состояние и трактуется
	 * как «включено», чтобы поведение уже работающих установок не изменилось.
	 *
	 * @returns {boolean}
	 */
	function resolveBlockPlaceOrder() {
		for( var i = 0; i < stores.length; i++ ) {
			if( stores[ i ].config && typeof stores[ i ].config.block_place_order !== 'undefined' ) {
				return !! stores[ i ].config.block_place_order
			}
		}
	...
	var blockPlaceOrder = resolveBlockPlaceOrder()

	// Единоразовое снятие блокировки при выключенном флаге — см. refreshGate().
	var placeOrderReleased = false
	 ...
	 *
	 * Issue #725: при выключенной настройке «Блокировать оформление заказа» это
	 * НЕ «включить кнопку» — это «не трогать её вовсе», иначе плагин рискует снова
	 * включить кнопку, которую легитимно отключил кто-то другой (например, платёжный
	 * метод, ещё не готовый к работе). Единственное исключение — САМЫЙ ПЕРВЫЙ вызов
	 * после загрузки скрипта: если кнопка уже отключена (наш же более ранний запуск,
	 * восстановление страницы браузером и т.п.), она снимается РОВНО ОДИН раз, и
	 * больше атрибут `disabled` этой веткой не трогается.
	 *
	...
	function refreshGate() {
		if( ! blockPlaceOrder ) {
			if( ! placeOrderReleased ) {
				placeOrderReleased = true
