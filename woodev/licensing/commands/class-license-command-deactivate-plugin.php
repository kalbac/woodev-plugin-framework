...
if ( ! class_exists( 'Woodev_License_Command_Deactivate_Plugin' ) ) :
	...
	class Woodev_License_Command_Deactivate_Plugin implements Woodev_License_Command {
		...
		/**
		 * Clears the pending remote-deactivation notice for a plugin that is (again)
		 * active.
		 *
		 ...
		 * not show a stale "you were disabled" banner for itself (relevant on a site
		 * with another active v2 plugin still rendering the banner). Removes the
		 * plugin's own entry from NOTICES_OPTION, deleting the whole option when it
		 * becomes empty. No-op when the plugin has no pending entry, so it adds no
		 * option churn on routine activations.
		 *
		 * @since 2.0.2
		 *
