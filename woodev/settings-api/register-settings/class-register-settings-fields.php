...
if ( ! class_exists( 'Woodev_Register_Settings_Fields' ) ) :

	class Woodev_Register_Settings_Fields extends Woodev_Register_Settings {
		...
		public function radio_button( $id, $label, $value, $section = '' ) {
			...
			$name_attr = esc_attr( $this->get_setting_id( $section ) );
			$id_attr   = esc_attr( $this->get_setting_id( $section ) . '[' . $id . ']' );
			$checked   = false;
		...
		public function selectbox( $id, $options, $classes = '' ) {
			...
			if ( empty( $options ) ) {
				if ( current_user_can( 'manage_options' ) ) {
					echo '<h6>' . __( 'Admin note: Option attributes are not set for all select controls.', 'woodev-plugin-framework' ) . '</h6>';
				}
				return;
			}
