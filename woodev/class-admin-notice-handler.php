...
if ( ! class_exists( 'Woodev_Admin_Notice_Handler' ) ) :
	...
	/**
	Woodev Admin Notice Handler Class
	...
	class Woodev_Admin_Notice_Handler {
		...
		public function __construct( $plugin ) {
			...
			add_action( 'wp_ajax_woodev_plugin_framework_' . $this->get_plugin()->get_id() . '_dismiss_notice', array( $this, 'handle_dismiss_notice' ) );
		}
		...
		$( '.js-woodev-plugin-framework-admin-notice' ).on( 'click.wp-dismiss-notice', '.notice-dismiss', function( e ) {

		var $notice = $( this ).closest( '.js-woodev-plugin-framework-admin-notice' );
		...
		$( 'a.js-woodev-plugin-framework-notice-dismiss' ).click( function( e ) {
		...
		ajaxurl,
		{
		action:    'woodev_plugin_framework_' + pluginID + '_dismiss_notice',
		messageid: messageID,
		nonce:     '<?php echo wp_create_nonce( 'woodev_dismiss_notice' ); ?>'
		}
		);
		}

		// move any delayed notices up into position .show();
		$( '.js-woodev-plugin-framework-admin-notice:hidden' ).insertAfter( '.js-wc-<?php echo esc_js( $plugin_slug ); ?>-admin-notice-placeholder' ).show();
		...
		public function handle_dismiss_notice() {
			...
			$this->dismiss_notice( sanitize_key( $_REQUEST['messageid'] ) );
		}
