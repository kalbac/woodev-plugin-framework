		...
		protected function maybe_add_user_profile_customer_id_fields( $gateway, $user ) {
            ...
            <table class="form-table">
				...
				foreach ( $environments as $environment_id => $environment_name ) :
                    ...
                    <tr>
                        <th>
                            <label for="<?php printf( '_wc_%s_customer_id_%s', $gateway->get_id(), $environment_id ); ?>"><?php echo count( $environments ) > 1 ? sprintf( 'ID клиента (%s)', $environment_name ) : 'ID клиента'; ?></label>
                        </th>
                        <td>
                            <input type="text"
                                   name="<?php printf( '_wc_%s_customer_id_%s', $gateway->get_id(), $environment_id ); ?>"
                                   value="<?php echo esc_attr( $gateway->get_customer_id( $user->ID, array(
                    ...
                    <table class="form-table">
                                    ...
                                    <ul style="margin:0;">
										...
										foreach ( $payment_tokens as $token ) :
                                            ...
                                            <li>
                                                ...
                                                (<?php printf( '%s заканчивается на %s истекает %s', $token->get_type_full(), $token->get_last_four(), $token->get_exp_month() . '/' . $token->get_exp_year() );
												echo( $token->is_default() ? ' <strong>Карта по умолчанию</strong>' : '' ); ?>
                                                )
                                                <a href="#" class="js-woodev-payment-token-delete"
                                                   data-payment_token="<?php echo $token->get_token(); ?>">Удалить</a>
                                            </li>
										<?php

										endforeach; ?>
                                    </ul>
                                    <input type="hidden" class="js-woodev-payment-tokens-deleted"
                                           name="wc_<?php echo $gateway->get_id(); ?>_payment_tokens_deleted_<?php echo $environment_id; ?>"
                                           value=""/>
								<?php endif; ?>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td style="padding-top:0px;">
                                <input type="text"
                                       name="wc_<?php echo $gateway->get_id(); ?>_payment_token_<?php echo $environment_id; ?>"
                                       placeholder="Токен" style="width:145px;"/>
								...
								<?php endif; ?>
                                <input type="text"
                                       name="wc_<?php echo $gateway->get_id(); ?>_payment_token_last_four_<?php echo $environment_id; ?>"
                                       placeholder="<?php printf( 'Последние 4 цифры', substr( date( 'Y' ) + 1, - 2 ) ); ?>"
                                       style="width:75px;"/>
                                <input type="text"
                                       name="wc_<?php echo $gateway->get_id(); ?>_payment_token_exp_date_<?php echo $environment_id; ?>"
                                       placeholder="<?php printf( 'Дата окончания', date( 'Y' ) + 1 ); ?>"
                                       style="width:155px;"/>
                                <br/>
