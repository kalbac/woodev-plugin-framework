	...
	abstract class Woodev_Payment_Gateway_Direct extends Woodev_Payment_Gateway {
		...
		public function validate_fields() {
			...
			if ( $this->supports_tokenization() ) {
				...
				if ( Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-payment-token' ) ) {

					// unknown token?
					if ( ! $this->get_payment_tokens_handler()->user_has_token( get_current_user_id(), Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-payment-token' ) ) ) {
						Woodev_Helper::wc_add_notice( esc_html__( 'Payment error, please try another payment method or contact us to complete your transaction.', 'woodev-plugin-framework' ), 'error' );
						$is_valid = false;
					}

					// Check the CSC if enabled
					if ( $this->is_credit_card_gateway() && $this->csc_enabled_for_tokens() ) {
						$is_valid = $this->validate_csc( Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-csc' ) ) && $is_valid;
					}
		...
		protected function validate_credit_card_fields( $is_valid ) {
			...
			$expiration_month = Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-exp-month' );
			$expiration_year  = Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-exp-year' );
			$expiry           = Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-expiry' );
			$csc              = Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-csc' );
		...
		protected function validate_credit_card_account_number( $account_number ) {
			...
			if ( empty( $account_number ) ) {
				...
				Woodev_Helper::wc_add_notice( esc_html__( 'Card number is missing', 'woodev-plugin-framework' ), 'error' );
				$is_valid = false;

			} else {

				if ( strlen( $account_number ) < 12 || strlen( $account_number ) > 19 ) {
					Woodev_Helper::wc_add_notice( esc_html__( 'Card number is invalid (wrong length)', 'woodev-plugin-framework' ), 'error' );
					$is_valid = false;
				}

				if ( ! ctype_digit( $account_number ) ) {
					Woodev_Helper::wc_add_notice( esc_html__( 'Card number is invalid (only digits allowed)', 'woodev-plugin-framework' ), 'error' );
					$is_valid = false;
				}
		...
		protected function validate_csc( $csc ) {
			...
			if ( ! empty( $csc ) ) {
				...
				// digit validation
				if ( ! ctype_digit( $csc ) ) {
					Woodev_Helper::wc_add_notice( esc_html__( 'Card security code is invalid (only digits are allowed)', 'woodev-plugin-framework' ), 'error' );
					$is_valid = false;
				}
		...
		public function process_payment( $order_id ) {
			...
			try {
				...
				if ( ( '0.00' === $order->payment_total && ! $this->transaction_forced() ) || $this->do_transaction( $order ) ) {
					 ...
					 *
					 * @param WC_Order $order order object
					 * @param Woodev_Payment_Gateway_Direct $gateway instance
					 */
					...
					try {
						...
						if ( $response->transaction_approved() ) {
							...
							if ( $response->get_status_message() ) {
								...
								$message = __( 'Unknown error', 'woodev-plugin-framework' );
							}
						...
						$message = sprintf(
							__( 'Payment method address could not be updated. %s', 'woodev-plugin-framework' ),
		 ...
		 *
		 * @param int|WC_Order $order order being processed
		 *
		 * @return WC_Order object with payment and transaction information attached
		 * @since 1.0.0
		 * @see Woodev_Payment_Gateway::get_order()
		 */
		public function get_order( $order ) {
			...
			if ( Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-account-number' ) && ! Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-payment-token' ) ) {
				...
				if ( $this->is_credit_card_gateway() ) {
					...
					$order->payment->card_type = Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-card-type' );
					$order->payment->exp_month = Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-exp-month' );
					$order->payment->exp_year  = Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-exp-year' );
					...
					// handle single expiry field formatted like "MM / YY" or "MM / YYYY"
					if ( Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-expiry' ) ) {
						list( $order->payment->exp_month, $order->payment->exp_year ) = array_map( 'trim', explode( '/', Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-expiry' ) ) );
					}
				...
				$token = $this->get_payment_tokens_handler()->get_token( $order->get_user_id(), Woodev_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-payment-token' ) );
		...
		protected function do_credit_card_transaction( $order, $response = null ) {
			...
			if ( $response->transaction_approved() ) {
				...
				/* translators: Placeholders: %1$s - payment method title, %2$s - environment ("Test"), %3$s - transaction type (authorization/charge), %4$s - card type (mastercard, visa, ...), %5$s - last four digits of the card */
					esc_html__( '%1$s %2$s %3$s Approved: %4$s ending in %5$s', 'woodev-plugin-framework' ),
		...
		protected function do_transaction( $order ) {
			...
			if ( $response->transaction_approved() || $response->transaction_held() ) {
					...
					try {
						...
						if ( ! $response->transaction_held() && ! ( $this->supports( self::FEATURE_CREDIT_CARD_AUTHORIZATION ) && $this->perform_credit_card_authorization( $order ) ) ) {
							...
							/* translators: Placeholders: %s - failure message */
								esc_html__( 'Tokenization Request Failed: %s', 'woodev-plugin-framework' ),
								$e->getMessage()
							);
							...
							/* translators: Placeholders: %1$s - payment method title, %2$s - failure message */
								esc_html__( '%1$s Tokenization Request Failed: %2$s', 'woodev-plugin-framework' ),
								$this->get_method_title(),
								$e->getMessage()
							);
				...
				if ( $response->transaction_held() || ( $this->supports( self::FEATURE_CREDIT_CARD_AUTHORIZATION ) && $this->perform_credit_card_authorization( $order ) ) ) {
					...
					$this->mark_order_as_held( $order, $this->supports( self::FEATURE_CREDIT_CARD_AUTHORIZATION ) && $this->perform_credit_card_authorization( $order ) ? esc_html__( 'Authorization only transaction', 'woodev-plugin-framework' ) : $response->get_status_message(), $response );
				}
		 ...
		 * stubbed in the WC_Payment_Gateway abstract class, but is called if the gateway declares support for it.
		 */
		...
		protected function do_add_payment_method_transaction( WC_Order $order ) {
			...
			if ( $response->transaction_approved() ) {
				...
				if ( $this->is_credit_card_gateway() ) {
					...
					$message = esc_html__( 'Nice! New payment method added.', 'woodev-plugin-framework' );
				}
		...
		protected function get_order_for_add_payment_method() {
			...
			$order->description = sprintf( esc_html__( '%1$s - Add Payment Method for %2$s', 'woodev-plugin-framework' ), sanitize_text_field( Woodev_Helper::get_site_name() ), $properties['billing_email'] );
		...
		/**
		 * Allow gateway implementations to add additional data to the data saved during the add payment method transaction
		 *
		 * @param Woodev_Payment_Gateway_API_Create_Payment_Token_Response $response create payment token response
		 *
		 * @return array
		 */
		...
		/**
		 * Returns true if a transaction should be forced (meaning payment processed even if the order amount is 0).
		 * This is useful mostly for testing situations
		 *
