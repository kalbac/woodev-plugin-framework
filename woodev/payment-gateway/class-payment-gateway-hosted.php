...
if ( ! class_exists( 'Woodev_Payment_Gateway_Hosted' ) ) :
	...
	abstract class Woodev_Payment_Gateway_Hosted extends Woodev_Payment_Gateway {
		 ...
		 *
		 * @see Woodev_Payment_Gateway::__construct()
		 */
		public function __construct( $id, $plugin, $args ) {
			...
			if ( ! has_action( 'woocommerce_api_' . strtolower( get_class( $this ) ), array(
				$this,
		...
		public function payment_fields() {
			...
			?>
            <style type="text/css">#payment ul.payment_methods li label[for='payment_method_<?php echo $this->get_id(); ?>'] img:nth-child(n+2) {
                margin-left: 1px;
            }</style><?php
		}
		 ...
		 *
		 * @see Woodev_Payment_Gateway_Hosted::use_form_post()
		 * @see Woodev_Payment_Gateway_Hosted::add_pay_page_handler()
		 * @see Woodev_Payment_Gateway::payment_page()
		 */
		 ...
		 * auto-submit this pay form, so as to make the checkout process as seamless as possile
		 *
		 ...
		 *
		 * @see Woodev_Payment_Gateway_Hosted::use_auto_form_post()
		 *
		 */
		 ...
		 *
		 * @return array
		 * @see Woodev_Payment_Gateway_Hosted::render_auto_post_form() for args
		 *
		 */
		 ...
		 * @see Woodev_Payment_Gateway_Hosted::get_hosted_pay_page_params()
		 */
		 ...
		 *
		 * @param Woodev_Payment_Gateway_API_Payment_Notification_Response $response response object
		 *
		 * @return WC_Order
		...
		protected function get_order_from_response( $response ) {
			...
			if ( ! $order ) {
				...
				throw new Woodev_Payment_Gateway_Exception( sprintf(
		 ...
		 *
		 * @param WC_Order $order the order object
		 * @param Woodev_Payment_Gateway_API_Payment_Notification_Response $response the response object
		 *
		 * @throws Woodev_Payment_Gateway_Exception
		 */
		protected function validate_transaction_response( $order, $response ) {
			...
			if ( ! $order->needs_payment() ) {
				...
				$order->add_order_note( sprintf( esc_html__( '%s duplicate transaction received', 'woodev-plugin-framework' ), $this->get_method_title() ) );
		 ...
		 * @param Woodev_Payment_Gateway_API_Payment_Notification_Response $response transaction response
		 *
		 * @throws Exception
		 */
		protected function process_transaction_response( $order, $response ) {
			...
			if ( $response->transaction_approved() || $response->transaction_held() ) {
				...
				if ( $this->supports_tokenization() && $this->tokenization_enabled() && $response instanceof Woodev_Payment_Gateway_Payment_Notification_Tokenization_Response ) {
		 ...
		 *
		 * @param WC_Order $order order object
		 * @param Woodev_Payment_Gateway_Payment_Notification_Tokenization_Response $response response object
		 *
				...
				if ( $response->payment_method_tokenized() ) {
					...
					if ( $response->tokenization_successful() && $this->get_payment_tokens_handler()->add_token( $order->get_user_id(), $token ) ) {
						...
						if ( $token->is_credit_card() ) {
							...
							$order->add_order_note( sprintf( __( '%1$s Payment Method Saved: %2$s ending in %3$s (expires %4$s)', 'woodev-plugin-framework' ),
							...
							$order->add_order_note( sprintf( __( '%1$s eCheck Payment Method Saved: %2$s account ending in %3$s', 'woodev-plugin-framework' ),
								$this->get_method_title(),
							...
							$order->add_order_note( sprintf( __( '%s Payment Method Saved', 'woodev-plugin-framework' ),
								$this->get_method_title()
							) );
						}
						...
						/* translators: Placeholders: %s - a failed tokenization API error */
							__( 'Tokenization failed. %s', 'woodev-plugin-framework' ),
		...
		/**
		 * Adds an order note, along with anything else required after an approved transaction.
		 * This is a generic, default approved handler.
		 *
		 * @param WC_Order $order the order object
		 * @param Woodev_Payment_Gateway_API_Payment_Notification_Response $response the response object
		 */
		 ...
		 *
		 * @param null|WC_Order $order Optional. The order object
		 * @param Woodev_Payment_Gateway_API_Payment_Notification_Response $response the response object
		 */
		 ...
		 *
		 * @return Woodev_Payment_Gateway_API_Payment_Notification_Response the response object
		 */
		...
		public function doing_transaction_response_handler() {
			return isset( $_REQUEST['wc-api'] ) && get_class( $this ) === sanitize_text_field( $_REQUEST['wc-api'] );
		}
		 ...
		 * @param string $message optional message string with a %s to hold the response data.  Defaults to 'Request %s' $response
		 */
		...
		/**
		 * Returns true if this gateway uses a form-post from the pay page to "redirect" to a hosted payment page
		 *
		 * @return boolean true if this gateway uses a form post, false if it redirects directly to the hosted pay page from checkout
		 */
		 ...
		 *
		 * @return boolean true if this gateway automatically posts to the remote processor server from the pay page
		 * @see Woodev_Payment_Gateway_Hosted::use_form_post()
		 */
