...
if ( ! class_exists( 'Woodev_Payment_Gateway_Abstract_Payment_Handler' ) ) :
	...
	abstract class Woodev_Payment_Gateway_Abstract_Payment_Handler {
		 ...
		 */
		protected function process_transaction_response( Woodev_Payment_Gateway_API_Response $response, WC_Order $order ) {
			...
			try {
				...
				if ( $response->transaction_approved() || $response->transaction_held() ) {
					...
					if ( $response->get_user_message() && $this->get_gateway()->is_detailed_customer_decline_messages_enabled() ) {
						$user_exception = new Woodev_Payment_Gateway_Exception( $response->get_user_message() );
					} else {
						$user_exception = null;
					}

					throw new Woodev_Payment_Gateway_Exception( $message, null, $user_exception );
				}
				...
				throw new Woodev_Payment_Gateway_Exception( $exception->getMessage(), $exception->getCode(), $exception->getPrevious() );
			}
		}
		 ...
		 */
		protected function validate_transaction_response( WC_Order $order, Woodev_Payment_Gateway_API_Response $response ) {
		...
		protected function process_order_transaction_held( WC_Order $order, Woodev_Payment_Gateway_API_Response $response ) {
			...
			if ( ! $user_message || ( $this->get_gateway()->supports_credit_card_authorization() && $this->get_gateway()->perform_credit_card_authorization( $order ) ) ) {
				$user_message = __( 'Your order has been received and is being reviewed. Thank you for your business.', 'woodev-plugin-framework' );
			}
			...
			$note_message = $this->get_gateway()->supports_credit_card_authorization() && $this->get_gateway()->perform_credit_card_authorization( $order ) ? __( 'Authorization only transaction', 'woodev-plugin-framework' ) : $response->get_status_message();
