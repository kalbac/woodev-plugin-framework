...
if ( ! class_exists( 'Woodev_Payment_Gateway_Capture_Handler' ) ) :
	 ...
	 */
	class Woodev_Payment_Gateway_Capture_Handler {


		/** @var Woodev_Payment_Gateway payment gateway instance */
		private $gateway;
		...
		public function perform_capture( WC_Order $order, $amount = null ) {
			...
			try {
				...
				if ( ! $this->get_gateway()->supports_credit_card_capture() ) {
					...
					wc_doing_it_wrong( __METHOD__, $message, '1.1.8' );

					throw new Woodev_Payment_Gateway_Exception( $message, 500 );
				}

				// don't try to capture failed/cancelled/fully refunded transactions
				if ( ! $this->is_order_ready_for_capture( $order ) ) {
					throw new Woodev_Payment_Gateway_Exception( __( 'Order cannot be captured', 'woodev-plugin-framework' ), 400 );
				}

				// don't re-capture fully captured orders
				if ( $this->has_order_authorization_expired( $order ) ) {
					throw new Woodev_Payment_Gateway_Exception( __( 'Transaction authorization has expired', 'woodev-plugin-framework' ), 400 );
				}

				// don't re-capture fully captured orders
				if ( $this->is_order_fully_captured( $order ) ) {
					throw new Woodev_Payment_Gateway_Exception( __( 'Transaction has already been fully captured', 'woodev-plugin-framework' ), 400 );
				}
				...
				if ( ! $response->transaction_approved() ) {
					...
					throw new Woodev_Payment_Gateway_Exception( $response->get_status_code() . ' - ' . $response->get_status_message() );
				}
				...
				if ( 500 !== $exception->getCode() ) {
					...
					/* translators: Placeholders: %1$s - payment gateway title (such as Authorize.net, Braintree, etc), %2$s - failure message. Definitions: "capture" as in capturing funds from a credit card. */
						__( '%1$s Capture Failed: %2$s', 'woodev-plugin-framework' ),
						$this->get_gateway()->get_method_title(),
		...
		public function do_capture_success( WC_Order $order, Woodev_Payment_Gateway_API_Response $response ) {
			...
			$this->get_gateway()->update_order_meta( $order, 'capture_total', Woodev_Helper::number_format( $total_captured ) );
			$this->get_gateway()->update_order_meta( $order, 'charge_captured', $this->get_gateway()->supports_credit_card_partial_capture() && $this->get_gateway()->is_partial_capture_enabled() && $total_captured < $this->get_order_capture_maximum( $order ) ? 'partial' : 'yes' );
		...
		public function is_order_fully_captured( WC_Order $order ) {
			...
			if ( ! $captured && $this->get_gateway()->supports_credit_card_partial_capture() && $this->get_gateway()->is_partial_capture_enabled() ) {
				$captured = (float) $this->get_gateway()->get_order_meta( $order, 'capture_total' ) >= $this->get_order_capture_maximum( $order );
			}
