...
if ( ! class_exists( 'Woodev_Payment_Gateway_Payment_Form' ) ) :
	...
	class Woodev_Payment_Gateway_Payment_Form extends Woodev_Script_Handler {
		...
		/** @var Woodev_Payment_Gateway gateway for this payment form */
		protected $gateway;

		/** @var array of Woodev_Payment_Gateway_Payment_Tokens, keyed by token ID */
		protected $tokens;
		 ...
		 * Sets up class.
		 *
		 * @param Woodev_Payment_Gateway|Woodev_Payment_Gateway_Direct $gateway gateway for form
		 */
		 ...
		 *
		 * @see Woodev_Payment_Gateway_Payment_Form::render()
		 */
		 ...
		 *
		 * @return array of Woodev_Payment_Gateway_Payment_Tokens, keyed by token ID
		 */
		 ...
		 *
		 * @return Woodev_Payment_Gateway|Woodev_Payment_Gateway_Direct
		 */
		...
		/**
		 * Returns true if tokenization is allowed for the payment form.
		 * This is true if both the gateway supports tokenization and it is enabled.
		 *
		 ...
		 * true when a subscription or pre-order is in the cart and the payment method must be tokenized.
		 *
		...
		public function get_payment_form_description_html() {
			...
			if ( $this->get_gateway()->is_test_environment() ) {
				echo '<p>' . esc_html__( 'TEST MODE ENABLED', 'woodev-plugin-framework' ) . '</p>';
			}
		...
		protected function get_manage_payment_methods_button_html() {
			 ...
			 *
			 * Allow actors to modify the "manage payment methods" button text rendered on the checkout page.
			 *
		...
		protected function get_saved_payment_method_html( $token ) {
			...
			// label
			$html .= sprintf( '<label class="woodev-payment-gateway-payment-form-saved-payment-method" for="wc-%s-payment-token-%s">', esc_attr( $this->get_gateway()->get_id_dasherized() ), esc_attr( $token->get_id() ) );
		...
		protected function get_save_payment_method_checkbox_html() {
			...
			if ( $this->tokenization_allowed() || $this->tokenization_forced() ) {
				...
				if ( $this->tokenization_forced() ) {
					...
					$html .= sprintf( '<input name="wc-%1$s-tokenize-payment-method" id="wc-%1$s-tokenize-payment-method" class="js-woodev-tokenize-payment method js-wc-%1$s-tokenize-payment-method" type="checkbox" value="true" style="width:auto;" %2$s/>', $this->get_gateway()->get_id_dasherized(), $checked ? 'checked="checked" ' : '' );
					...
					$html .= sprintf( '<label for="wc-%s-tokenize-payment-method" style="display:inline;">%s</label>', $this->get_gateway()->get_id_dasherized(), apply_filters( 'wc_' . $this->get_gateway()->get_id() . '_tokenize_payment_method_text', esc_html__( 'Securely Save to Account', 'woodev-plugin-framework' ) ) );
					$html .= '</p><div class="clear"></div>';
				}
			}
		...
		public function render_fieldset_start() {
			...
			printf( '<fieldset id="wc-%s-%s-form" aria-label="%s"><legend style="display:none;">%s</legend>', esc_attr( $this->get_gateway()->get_id_dasherized() ), esc_attr( $this->get_gateway()->get_payment_type() ), esc_attr__( 'Payment Info', 'woodev-plugin-framework' ), esc_attr__( 'Payment Info', 'woodev-plugin-framework' ) );
		...
		/**
		 * Render the payment form closing fieldset tag, clearing div, and "save payment method" checkbox
		 *
		 ...
		 * However, in the circumstance this doesn't trigger, {@see Woodev_Payment_Gateway_Payment_Form::maybe_render_js()} hooked to footer.
		 * This may happen when the customer reaches checkout with a $0 value order.
		 *
		 * @see Woodev_Payment_Gateway_Payment_Form::get_safe_handler_js()
		 */
