...
if ( ! class_exists( 'Woodev_Payment_Gateway_Payment_Tokens_Handler' ) ) :

/**
 * Handle the payment tokenization related functionality.
 *
 */
class Woodev_Payment_Gateway_Payment_Tokens_Handler {

	/** @var string the gateway environment ID */
	protected $environment_id;

	/** @var array|Woodev_Payment_Gateway_Payment_Token[] array of cached user id to array of Woodev_Payment_Gateway_Payment_Token token objects */
	protected $tokens;
	 ...
	 * @param Woodev_Payment_Gateway_API_Create_Payment_Token_Response|null $response payment token API response, or null if the request should be made
	 * @param string $environment_id optional environment ID, defaults to the current environment
	 * @return WC_Order order object
	 * @throws Woodev_Plugin_Exception on transaction failure
	 */
	public function create_token( WC_Order $order, $response = null, $environment_id = null ) {
		...
		if ( $response->transaction_approved() ) {
			...
			//generate hash from order address data
			$address_hash = md5( json_encode( array_filter( array(
			...
			$token->set_billing_hash( $address_hash );
			...
			if ( $response->get_status_code() && $response->get_status_message() ) {
				...
				$message = sprintf( esc_html__( 'Status code %1$s: %2$s', 'woodev-plugin-framework' ), $response->get_status_code(), $response->get_status_message() );
			} elseif ( $response->get_status_code() ) {
				/* translators: Placeholders: %s - payment request response status code */
				$message = sprintf( esc_html__( 'Status code: %s', 'woodev-plugin-framework' ), $response->get_status_code() );
			} elseif ( $response->get_status_message() ) {
				/* translators: Placeholders: %s - payment request response status message */
				$message = sprintf( esc_html__( 'Status message: %s', 'woodev-plugin-framework' ), $response->get_status_message() );
			} else {
				$message = esc_html__( 'Unknown Error', 'woodev-plugin-framework' );
			}
	 ...
	 * @param Woodev_Payment_Gateway_Payment_Token $token the token
	...
	/**
	 * Returns the payment token object identified by $token from the user identified by $user_id
	 *
	 * @since 1.0.0
	 *
	 ...
	 * @param string $token payment token
	 * @param string|null $environment_id optional environment id, defaults to plugin current environment
	 * @return Woodev_Payment_Gateway_Payment_Token payment token object or null
	 */
	 ...
	 * @param Woodev_Payment_Gateway_Payment_Token $token token to update
	 * @param string|null $environment_id optional environment ID, defaults to plugin current environment
	 ...
	 *
	 * @param int $user_id user identifier
	 * @param Woodev_Payment_Gateway_Payment_Token|string $token the payment token to delete
	 * @param string|null $environment_id optional environment id, defaults to plugin current environment
	 ...
	 * @return bool
	 */
	public function should_delete_token( Woodev_Payment_Gateway_Payment_Token $token, Woodev_Payment_Gateway_API_Response $response ) {
		return false;
	}
	 ...
	 * @return bool
	 */
	public function delete_token( $user_id, Woodev_Payment_Gateway_Payment_Token $token, $environment_id = null ) {
	 ...
	 *
	 * @param int $user_id user identifier
	 * @param Woodev_Payment_Gateway_Payment_Token|string $token the token to make default
	 * @param string|null $environment_id optional environment id, defaults to plugin current environment
	...
	/**
	 * Gets the available payment tokens for a user as an associative array of payment token to Woodev_Payment_Gateway_Payment_Token
	 *
	 * @since 1.0.0
	 *
	 * @param int $user_id WordPress user identifier, or 0 for guest
	...
	public function get_tokens( $user_id, $args = array() ) {
		 ...
		 *
		 * @param array $tokens array of Woodev_Payment_Gateway_Payment_Tokens
		 * @param Woodev_Payment_Gateway $gateway class instance
		 */
	 ...
	 *
	 * @return Woodev_Payment_Gateway_Admin_Payment_Token_Editor
	 */
	public function get_token_editor() {
		return new Woodev_Payment_Gateway_Admin_Payment_Token_Editor( $this->get_gateway() );
	}
	 ...
	 *
	 * @param int $user_id WordPress user identifier, or 0 for guest
	 * @param string|Woodev_Payment_Gateway_Payment_Token $token payment token
	 * @param string|null $environment_id optional environment id, defaults to plugin current environment
	 * @return bool
	 */
	 ...
	 *
	 * Whether requested by customer or otherwise forced. This parameter is passed from the checkout page/payment form.
	 *
	 * @since 1.0.0
	 *
	 * @return bool
	...
	public function should_tokenize() {
		return Woodev_Helper::get_posted_value( 'wc-' . $this->get_gateway()->get_id_dasherized() . '-tokenize-payment-method' ) && ! Woodev_Helper::get_posted_value( 'wc-' . $this->get_gateway()->get_id_dasherized() . '-payment-token' );
	}
	...
	public function tokenization_forced() {
		 ...
		 *
		 * Allow actors to indicate that tokenization should be forced for the current checkout.
		 *
		 * @since 1.0.0
		 *
	...
	protected function get_transient_key( $user_id = null ) {
		...
		// ex: woodev_tokens_<md5 hash of gateway_id, user ID, and environment ID>
		$key = sprintf( 'woodev_tokens_%s', md5( $this->get_gateway()->get_id() . '_' . $user_id . '_' . $this->get_environment_id() ) );
		 ...
		 * @param Woodev_Payment_Gateway $this direct gateway class instance
		 */
	 ...
	 * to their account
	 *
	 * @param Woodev_Payment_Gateway_Payment_Token $token the payment token being saved
	 * @return string
	 */
	protected function get_order_note( $token ) {
		...
		if ( $gateway->is_credit_card_gateway() ) {
			...
			$message = sprintf( __( '%1$s Payment Method Saved: %2$s ending in %3$s (expires %4$s)', 'woodev-plugin-framework' ),
			...
			$message = sprintf( __( '%1$s eCheck Payment Method Saved: %2$s account ending in %3$s', 'woodev-plugin-framework' ),
				$gateway->get_method_title(),
	 ...
	 * @since 1.0.0
	 *
	 * @param array $tokens array of Woodev_Payment_Gateway_Payment_Token tokens
	 ...
	 *
	 * @return Woodev_Payment_Gateway gateway instance
	 */
