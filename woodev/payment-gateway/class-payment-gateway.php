...
if ( ! class_exists( 'Woodev_Payment_Gateway' ) ) :
	...
	abstract class Woodev_Payment_Gateway extends WC_Payment_Gateway {
		...
		/** Loans payment type */
		const PAYMENT_TYPE_LOANS = 'loans_transfer';
		...
		/** @var Woodev_Payment_Gateway_Capture_Handler capture handler instance */
		 ...
		 * Args:
		 *
		 * + `method_title` - string admin method title, ie 'Tinkoff', defaults to 'Settings'
		...
		protected function enqueue_gateway_assets() {
			...
			$js_path  = $this->get_plugin()->get_plugin_path() . '/assets/js/frontend/' . $handle . '.js';
			$css_path = $this->get_plugin()->get_plugin_path() . '/assets/css/frontend/' . $handle . '.css';

			// JS
			if ( is_readable( $js_path ) ) {

				$js_url = $this->get_plugin()->get_plugin_url() . '/assets/js/frontend/' . $handle . '.js';
			...
			if ( is_readable( $css_path ) ) {
				...
				$css_url = $this->get_plugin()->get_plugin_url() . '/assets/css/frontend/' . $handle . '.css';
		 ...
		 *
		 * @return Woodev_Payment_Gateway_Payment_Form
		 */
		protected function init_payment_form_instance() {
			return new Woodev_Payment_Gateway_Payment_Form( $this );
		}
		...
		protected function get_order_button_text() {
			...
			$text = $this->is_hosted_gateway() ? esc_html__( 'Continue to Payment', 'woodev-plugin-framework' ) : esc_html__( 'Place order', 'woodev-plugin-framework' );
		...
		public function payment_page( $order_id ) {
			echo '<p>' . esc_html__( 'Thank you for your order.', 'woodev-plugin-framework' ) . '</p>';
		}
		 ...
		 * @see Woodev_Payment_Gateway_Payment_Form class
		 */
		 ...
		 *
		 * @return Woodev_Payment_Gateway_Payment_Form|null
		 */
		 ...
		 *
		 * @return Woodev_Payment_Gateway_Payment_Tokens_Handler
		 */
		protected function build_payment_tokens_handler() {
			return new Woodev_Payment_Gateway_Payment_Tokens_Handler( $this );
		}
		 ...
		 *
		 * @return array $form_fields gateway form fields
		 * @see Woodev_Payment_Gateway::$inherit_settings
		 * @since 1.0.0
		 * @see Woodev_Payment_Gateway::$shared_settings
		 */
		...
		public function get_icon() {
			...
			if ( ! $icon && $this->supports_card_types() && $this->get_card_types() ) {
				...
				$icon .= '<div class="woodev-payment-gateway-card-icons">';
				...
				foreach ( $this->get_card_types() as $card_type ) {
					...
					if ( $url = $this->get_payment_method_image_url( $card_type ) ) {
						$icon .= sprintf( '<img src="%s" alt="%s" class="woodev-payment-gateway-icon wc-%s-payment-gateway-icon" width="40" height="25" style="width: 40px; height: 25px;" />', esc_url( $url ), esc_attr( $card_type ), esc_attr( $this->get_id_dasherized() ) );
					}
				}
		...
		public function get_order( $order ) {
			...
			$order->description = sprintf( esc_html__( '%1$s - Order %2$s', 'woodev-plugin-framework' ), wp_specialchars_decode( Woodev_Helper::get_site_name(), ENT_QUOTES ), $order->get_order_number() );
		 ...
		 * @throws Exception
		 */
		protected function complete_payment( WC_Order $order, Woodev_Payment_Gateway_API_Response $response ) {
			...
			if ( $response->transaction_held() || ( $this->supports_credit_card_authorization() && $this->perform_credit_card_authorization( $order ) ) ) {
				...
				$message = $this->supports_credit_card_authorization() && $this->perform_credit_card_authorization( $order ) ? __( 'Authorization only transaction', 'woodev-plugin-framework' ) : $response->get_status_message();
		...
		public function init_capture_handler() {
			$this->capture_handler = new Woodev_Payment_Gateway_Capture_Handler( $this );
		}

		/**
		 * Gets the capture handler instance.
		 *
		 * @return Woodev_Payment_Gateway_Capture_Handler
		 */
		...
		public function get_order_for_capture( $order, $amount = null ) {
			...
			if ( ! $amount ) {
				$amount = $order->get_total() - (float) $total_captured;
			}
			...
			$order->capture->description = sprintf( esc_html__( '%1$s - Capture for Order %2$s', 'woodev-plugin-framework' ), wp_specialchars_decode( Woodev_Helper::get_site_name() ), $order->get_order_number() );
		...
		public function process_refund( $order_id, $amount = null, $reason = '' ) {
			...
			try {
				...
				$response = $this->get_api()->refund( $order );
				...
				if ( $response->transaction_approved() ) {
					 ...
					 *
					 * @param WC_Order $order order object
					 * @param Woodev_Payment_Gateway $gateway payment gateway instance
					 */
		...
		protected function get_order_for_refund( $order, $amount, $reason ) {
			...
			/* translators: Placeholders: %1$s - site title, %2$s - order number */
			$order->refund->reason = $reason ?: sprintf( esc_html__( '%1$s - Refund for Order %2$s', 'woodev-plugin-framework' ), esc_html( Woodev_Helper::get_site_name() ), $order->get_order_number() );
		...
		protected function get_refund_failed_wp_error( $error_code, $error_message ) {
			...
			if ( $error_code ) {
				$message = sprintf(
				/* translators: Placeholders: %1$s - payment gateway title (such as Tinkoff, Robokassa etc), %2$s - error code, %3$s - error message */
					esc_html__( '%1$s Refund Failed: %2$s - %3$s', 'woodev-plugin-framework' ),
					$this->get_method_title(),
					$error_code,
					$error_message
				);
				...
				$message = sprintf(
				/* translators: Placeholders: %1$s - payment gateway title (such as Tinkoff, Robokassa etc), %2$s - error message */
					esc_html__( '%1$s Refund Failed: %2$s', 'woodev-plugin-framework' ),
					$this->get_method_title(),
					$error_message
				);
			}
		...
		public function mark_order_as_refunded( $order ) {
			...
			/* translators: Placeholders: %s - payment gateway title (such as Tinkoff, Robokassa etc) */
			$order_note = sprintf( esc_html__( '%s Order completely refunded.', 'woodev-plugin-framework' ), $this->get_method_title() );
		...
		protected function process_void( WC_Order $order ) {
			...
			if ( $order->refund->amount != $order->get_total() ) {
				return new WP_Error( 'wc_' . $this->get_id() . '_void_error', esc_html__( 'Oops, you cannot partially void this order. Please use the full order amount.', 'woodev-plugin-framework' ), 500 );
			}
		...
		protected function get_void_failed_wp_error( $error_code, $error_message ) {
			...
			if ( $error_code ) {
				...
				/* translators: Placeholders: %1$s - payment gateway title, %2$s - error code, %3$s - error message. Void as in to void an order. */
					esc_html__( '%1$s Void Failed: %2$s - %3$s', 'woodev-plugin-framework' ),
					$this->get_method_title(),
					$error_code,
					$error_message
				);
				...
				/* translators: Placeholders: %1$s - payment gateway title, %2$s - error message. Void as in to void an order. */
					esc_html__( '%1$s Void Failed: %2$s', 'woodev-plugin-framework' ),
					$this->get_method_title(),
					$error_message
				);
			}
		...
		protected function get_order_with_unique_transaction_ref( $order ) {
			...
			$order->unique_transaction_ref = ltrim( $order->get_order_number(), esc_html_x( '#', 'hash before order number', 'woodev-plugin-framework' ) ) . ( $retry_count > 0 ? '-' . $retry_count : '' );
		...
		protected function do_transaction_failed_result( WC_Order $order, Woodev_Payment_Gateway_API_Response $response ) {
			...
			if ( $response->get_status_code() && $response->get_status_message() ) {
				...
				$order_note = sprintf( esc_html__( 'Status code %1$s: %2$s', 'woodev-plugin-framework' ), $response->get_status_code(), $response->get_status_message() );
			} elseif ( $response->get_status_code() ) {
				/* translators: Placeholders: %s - status code */
				$order_note = sprintf( esc_html__( 'Status code: %s', 'woodev-plugin-framework' ), $response->get_status_code() );
			} elseif ( $response->get_status_message() ) {
				/* translators: Placeholders; %s - status message */
				$order_note = sprintf( esc_html__( 'Status message: %s', 'woodev-plugin-framework' ), $response->get_status_message() );
			}

			// add transaction id if there is one
			if ( $response->get_transaction_id() ) {
				$order_note .= ' ' . sprintf( esc_html__( 'Transaction ID %s', 'woodev-plugin-framework' ), $response->get_transaction_id() );
			}
		...
		public function add_transaction_data( $order, $response = null ) {
			...
			if ( $this->is_credit_card_gateway() ) {
				...
				if ( $response instanceof Woodev_Payment_Gateway_API_Authorization_Response ) {
		 ...
		 * @throws Exception
		 */
		public function get_credit_card_transaction_approved_message( WC_Order $order, Woodev_Payment_Gateway_API_Response $response ) {
			...
			/* translators: Placeholders: %1$s - payment method title, %2$s - environment ("Test"), %3$s - transaction type (authorization/charge) */
				__( '%1$s %2$s %3$s Approved', 'woodev-plugin-framework' ),
				$this->get_method_title(),
			 ...
			 *
			 * Allow actors to modify the order note added when a Credit Card transaction is approved.
			 *
		 ...
		 * @return string
		 */
		public function get_loans_transaction_approved_message( WC_Order $order, Woodev_Payment_Gateway_API_Response $response ) {

			$message = sprintf( __( '%s the loan was successfully Approved', 'woodev-plugin-framework' ), $this->get_method_title() );

			if ( ! empty( $order->payment->loan_type ) ) {
				$message .= '. ' . sprintf( esc_html__( 'The loan type: %s', 'woodev-plugin-framework' ), Woodev_Payment_Gateway_Helper::get_loan_type_name( $order->payment->loan_type ) );
			}
		...
		public function mark_order_as_held( $order, $message, $response = null ) {
			...
			$order_note = sprintf( esc_html__( '%1$s Transaction Held for Review (%2$s)', 'woodev-plugin-framework' ), $this->get_method_title(), $message );
			 ...
			 * @param Woodev_Payment_Gateway_API_Response $response instance
			...
			if ( ! $user_message || ( $this->supports_credit_card_authorization() && $this->perform_credit_card_authorization( $order ) ) ) {
				$user_message = esc_html__( 'Your order has been received and is being reviewed. Thank you for your business.', 'woodev-plugin-framework' );
			}
		...
		/**
		 * Maybe render custom order received text on the thank you page when an order is held
		 *
		...
		public function mark_order_as_failed( $order, $error_message, $response = null ) {
			...
			$order_note = sprintf( esc_html__( '%1$s Payment Failed (%2$s)', 'woodev-plugin-framework' ), $this->get_method_title(), $error_message );
			...
			if ( ! $user_message ) {
				$user_message = esc_html__( 'An error occurred, please try again or try an alternate form of payment.', 'woodev-plugin-framework' );
			}
		...
		public function mark_order_as_cancelled( $order, $message, $response = null ) {
			...
			$order_note = sprintf( esc_html__( '%1$s Transaction Cancelled (%2$s)', 'woodev-plugin-framework' ), $this->get_method_title(), $message );
		...
		/**
		 * Returns true if this is a credit card gateway which supports authorization transactions
		 *
		...
		/**
		 * Returns true if this is a credit card gateway which supports charge transactions
		 *
		...
		/**
		 * Return the authorization time window in hours. An authorization is considered expired if it is older than this.
		 *
		 * 30 days (720 hours) is the standard authorization window. Individual gateways can override this as necessary.
		 *
		 * @return int hours
		 */
		...
		/**
		 * Returns true if the gateway supports card_types: allows the admin to configure card type icons to display at checkout
		 *
		...
		/**
		 * Adds any card types form fields, allowing the admin to configure the card types icons displayed during checkout
		 *
		 * @param array $form_fields gateway form fields
		 *
		 * @return array $form_fields gateway form fields
		...
		protected function add_card_types_form_fields( $form_fields ) {
			...
			$form_fields['card_types'] = array(
				...
				/* translators: Placeholders: %1$s - <strong> tag, %2$s - </strong> tag */
					__( 'This setting %1$sdoes not%2$s change which card types the gateway will accept. Accepted cards are configured from your payment processor account.', 'woodev-plugin-framework' ),
					'<strong>',
					'</strong>'
				),
		...
		public function get_available_card_types() {
			...
			if ( ! isset( $this->available_card_types ) ) {
				...
				$this->available_card_types = array(
					...
					'DISC'   => esc_html_x( 'Discover', 'credit card type', 'woodev-plugin-framework' ),
