...
> `show_if` v1 shipped s40 (individual fields; grammar `relation` + flat `{setting, operator, value}`; operators `=`/`!=`/`in`/`not_in`; array-or-callback declaration; server strip via `filter_visible_values()`; both surfaces). Contract locked in **ADR-008**; design in `archive/specs/2026-07-05-conditional-fields-design.md`.
...
> Source: `docs-internal/archive/reviews/fable5-architecture-review-2026-06-10.md` (single Fable 5 agent, fresh-eyes architecture/direction review). **Operator decision (s5): record now, do not fix now.** Each item carries a trigger-stage; convert to an autodev atomic task when its trigger fires. **Verification:** B-1/B-2/B-3 verified against source this session (file:line in the report); B-4…B-12 are plausible-pending-verification — **re-verify before acting** (never apply an external audit blindly).
>
...
### SP-2-DEF — Secret wipe / disconnect affordance — ✅ SHIPPED (s41, PR #100 `f9c2269`)
- **Shipped per-field** (operator chose per-field over a block-level «Отключить»; block-level can layer on later). New `SecretControl` (shared `control-field`): on a stored (`is_set`), non-constant sensitive field a «Очистить сохранённое» link stages an empty edit → a distinct pending-clear notice «Сохранённый секрет будет удалён при сохранении» + «Отменить» (drops that single edit via app-level `onFieldRevert`); the wipe persists on Save. **Client-only** — see gotcha `settings-sensitive-secret-empty-skip-is-client-side` (the server save already persists `''` → clears). Threaded `hasEdit`/`onRevert` through SectionView + ConnectionBlock. Browser-verified on `:8888` + Codex critic clean.
- **Deferred (still open):** block-level «Отключить» (wipe all a connection block's secrets at once) — add when a real carrier needs it.

### SP-4 — DaData service seam — DEFERRED until §8 (checkout field layer) (decided s41)
- **Decision (s41):** do **NOT** build SP-4 now. The only thing buildable without §8 already exists — `Settings_Page_Registry::register_service(Settings_Provider)` (SP-1) lets a framework service (DaData) register its own settings tab, and SP-2 masks its token/secret. Everything else in SP-4 is the **behavioral seam** (`suggest(field, query)` / `normalize(address)` + a reusable client widget), which has **no consumer** until §8 (checkout field registry) exists, and whose contract must be co-designed with §8 + a detailed DaData domain review (operator: "от этого сервиса многое зависит в плагинах доставки"). Building an empty seam now = premature interface lock ahead of a real need (violates the same YAGNI/mechanism-ahead-of-need rule we hold elsewhere).
- **Target when built (operator, s41):** the **full address service** — checkout address autocomplete (cascade region→city→address, index autofill, geo-IP city) **and** server-side normalization/ФИАС — with DaData domain specifics (endpoints, response format, tokens) living in the **plugin**, not the framework.
- **When:** after §8 (SP-3-checkout field layer) lands; SP-4's own brainstorm → spec → plan, grounded on §8 as the first consumer.
