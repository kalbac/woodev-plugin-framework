...
**Branch:** `main` (S3.2 merged; next work branches off fresh `main`) · **Baselines:** `platform-v2-pivot-baseline`, `platform-v2-pre-refactor`
**Last updated:** 2026-06-11 (session 8 — S3.2 implemented + **PR #31 MERGED** `f7d29f3`; all GH Actions green)
...
▶️ **Next: S3.3 — built-in webhooks (s8 tasks)** per `platform-v2-s3-licensing-webhooks-spec.md` — its §9 BLOCKING protocol-hardening checklist is the task list; includes the §4 Ed25519 signing implementation + the B-3 updater rework, and the cross-repo step of capturing the woodev-core PRODUCTION public key. S3.2's `Woodev_REST_V1_Registrar` is the registration seam for `woodev/v1/license-command`.
...
| S3 Licensing | `is_need_license` → modern UI → webhooks | 🟢 **in progress** — sub-stage 1 (`is_need_license`) MERGED `61006c3`; **sub-stage 2 (UI) MERGED PR #31 `f7d29f3`**; sub-stage 3 (webhooks, DRAFT spec) is next | `platform-v2-s3-licensing-need-license-spec.md`, `platform-v2-s3-licensing-ui-spec.md`, **`platform-v2-s3-licensing-webhooks-spec.md`** |
