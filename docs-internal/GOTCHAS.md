...
- [tooling/parallel-agents] **Codex's own shell tool is a LINUX shell (the Orca terminal is not), so a worktree's `gitdir: D:/…` fails every git command — rewrite it relative, from the worktree root. `worktree.useRelativePaths` looks like the fix and makes git 2.43 refuse the WHOLE repo.** → [codex-in-wsl-needs-a-relative-gitdir](gotchas/codex-in-wsl-needs-a-relative-gitdir.md) (s107)
