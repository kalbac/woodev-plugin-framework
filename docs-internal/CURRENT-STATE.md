...
- **s26 part-2 (DONE — SHIPPED, PR #77 `086585c`):** #8 install purchased plugin from connector. Rig e2e PASSED + operator browser-verified; Codex no CRITICAL/HIGH (MEDIUM+LOW fixed). **Connector deployed to prod woodev.ru** (operator). Version NOT bumped (still 2.0.1 unreleased, `@since 2.0.2`). Spinner overlay + filterable install timeout (30s) included. **725 unit.** Next: operator's new idea (TBD) / backlog.
