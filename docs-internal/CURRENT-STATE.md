...
> Last updated: 2026-06-13 (session 11: LIVE remote-deactivation e2e proven on BOTH channels — push (prod woodev.ru + public staging) and pull (local two-stack rig) — plus replay-reject (HTTP 410). Found+fixed a release-blocking WSOD (box-packer interface unwired). Framework **2.0.1** merged to main (PR #41 + test-fix PR #42, `dd63e5f`, CI green 603 tests). **Open:** (1) v2.0.1 release NOT published — auto-release missed (version-change in #41 had red tests; #42 fixed tests w/o version-change); operator deferred. (2) Operator manual run found 3 remote-deactivation UX gaps (B-13/14/15, findings doc 2026-06-13). **Next (s12):** fix B-13/14/15 (notice lifecycle + issuer button) — THEN edostavka pilot (audit-first).)
