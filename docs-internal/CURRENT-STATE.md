...
> Last updated: 2026-06-21 (session 27 — framework runtime autoloader + plugin type via `extends`; PLANS §5 resolved as variant C; `capabilities` removed; PR #78 MERGED; autonomous overnight run).
...
- **⚠️ NEW pre-release blocker (B-2 now has teeth):** removing `capabilities` evolved the loader protocol. If an **older v2** copy wins the bootstrap rendezvous in a mixed v2 fleet, its resolver lacks the autoloader → a new-protocol plugin's `extends` fatals. **Not a live risk** (v2 unreleased; no v2 plugin shipped with the old protocol), but **B-2 loader-protocol forward-tolerance MUST be designed before any v2 plugin ships.** See SESSION-LOG s27 + Big ones + gotcha `framework-classmap-autoload-vendored-boot`.
...
`composer check` green at s27: **729 unit tests** / 2090 assertions (66 skipped), 41 integration (baseline). Keep green after each change.
