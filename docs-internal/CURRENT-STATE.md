...
> Last updated: 2026-06-13 (session 12: remote-deactivation UX gaps B-13/14/15 **all resolved + rig-verified vs real WC 10.8.1**. (A) `handle_activation()` clears stale notice + WC note on reactivation; (B) operator-picked WC Admin Notes breadcrumb (`Woodev_Notes_Helper::add_note`) written by the deactivate command AFTER `handle_deactivation`'s bulk-delete so it survives — solves the single-v2-plugin render gap; (C) deactivator «Отменить»→«Снять с доставки» wording by delivery state + re-deactivation-after-terminal verified. Framework PR #44 merged (`21bb436`, 608 unit, CI green, Codex-criticked); deactivator committed local `28af8b9`. **Open:** v2.0.1 release STILL NOT published (operator deferred); s12 APIs are `@since 2.0.2`, VERSION still 2.0.1 — release decision pending. WC-note is ADDITIVE to the banner (flagged in PR #44). **Next (s13):** edostavka pilot — audit-first (request operator's shipping-module nuance list → audit `woodev/shipping-method/` → migration spec + data-preservation contract test B-11).)

## Session 12 (2026-06-13) — remote-deactivation UX (B-13/14/15) all resolved; framework PR #44 merged
- **Finding A (REAL):** `Woodev_Lifecycle::handle_activation()` clears the plugin's own `woodev_license_remote_deactivation_notices` entry (deletes option when empty) + its WC inbox note, on a genuine reactivation transition, via new `Woodev_License_Command_Deactivate_Plugin::clear_remote_deactivation_artifacts()`.
- **Finding B (design → WC Admin Notes):** single-v2-plugin sites can't render the `admin_notices` banner post-deactivation (framework not loaded). New `Woodev_Notes_Helper::add_note()`; the deactivate command writes a WC inbox breadcrumb AFTER `deactivate_plugins()`/`handle_deactivation` so it survives the source-based bulk-delete. WC renders it independent of the plugin → survives. Additive to the banner (operator to decide sole-channel later).
...
- **Verified** on the reused two-stack rig vs real WC 10.8.1 (note create/survive/clear; issuer re-deactivation). Codex inline critic: 2 real fixes applied (best-effort try/catch on WC note delete; `add_note` returns `(bool) save()`), re-criticked clean.
- **Gotchas:** [[is-enhanced-admin-available-always-true]] (returns true unconditionally — guard WC code on `class_exists('\Automattic\WooCommerce\Admin\Notes\Note')`), [[wc-note-breadcrumb-survives-deactivation]] (ordering: create the note AFTER `handle_deactivation` bulk-deletes by source).
