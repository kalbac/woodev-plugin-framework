...
> Last updated: 2026-06-13 (session 12: remote-deactivation UX gaps B-13/14/15 resolved + rig-verified vs real WC 10.8.1. (A) `handle_activation()` clears the stale notice option entry on reactivation (KEPT); (B) WC Admin Notes breadcrumb was tried then **REVERTED per operator** — a single-v2-plugin site intentionally shows NO banner (kill-switch targets violators; banner shows only with ≥2 active v2 plugins); (C) deactivator «Отменить»→«Снять с доставки» wording by delivery state + re-deactivation-after-terminal verified. Framework PR #44 merged (`21bb436`); B-revert PR #46. Deactivator local `28af8b9`. **Open:** v2.0.1 release STILL NOT published — operator: do NOT bump version per-change; release deferred. **Next (s13):** framework **grooming** (operator: edostavka pilot DEFERRED — "too early"; bring the framework to a clean/working state first, fix framework bugs during the eventual pilot). s13 starts by asking the operator for the grooming priority list; candidates = clean-break Phase 3 shim deletion, PHPStan baseline cleanup, payment-gateway trait extraction, `@since`/`array()`→`[]` sweep.)

## Session 12 (2026-06-13) — remote-deactivation UX (B-13/14/15) all resolved; framework PR #44 merged
- **Finding A (REAL, KEPT):** `Woodev_Lifecycle::handle_activation()` clears the plugin's own `woodev_license_remote_deactivation_notices` entry (deletes option when empty) on a genuine reactivation transition, via new `Woodev_License_Command_Deactivate_Plugin::clear_remote_deactivation_artifacts()` — kills the stale "you were disabled" banner on a site with another active v2 plugin still rendering it.
- **Finding B (tried WC Admin Notes → REVERTED per operator, PR #46):** single-v2-plugin sites can't render the `admin_notices` banner post-deactivation (framework not loaded). Initially shipped a WC inbox breadcrumb (PR #44); operator decided a single-v2-plugin site showing no banner is **fine** (the kill-switch targets violators — nothing lost) and two surfaces on a multi-plugin site is redundant. Reverted to banner-only: it shows only when ≥1 OTHER active v2 plugin renders it (= "more than one v2 plugin"). `Woodev_Notes_Helper::add_note()` + the breadcrumb code removed.
- **Finding C (woodev_theme deactivator):** «Отменить» (queued) vs «Снять с доставки» (delivered_pending_ack/failed_retryable) wording by delivery state; re-deactivation-after-terminal verified (cancel stuck row → `can_deactivate` re-opens → `issue()` allows). The "stuck on Отменить" was a pull-only rig artifact.
- **Verified** on the reused two-stack rig vs real WC 10.8.1; `composer check` green (607 unit after revert). Codex inline critic on the original feature (2 fixes applied, re-criticked clean) before the operator chose to drop B.
- **Gotcha:** [[is-enhanced-admin-available-always-true]] (returns true unconditionally — guard WC code on `class_exists('\Automattic\WooCommerce\Admin\Notes\Note')`; permanent trap for any WC-gated code).

## Session 11 (2026-06-13) — live remote-deactivation e2e + 2.0.1 (PR #41)
- **Full command cycle proven LIVE on real code, both channels:** PUSH (prod woodev.ru issues → POST to public `pochta.wootest.ru` → execute → ack → "выполнена (push: executed)"); PULL (local rig, real `validate_license`: knock → gating → issue → pull → Ed25519 verify → deactivate → notice → ack `row=executed` → **replay POST = HTTP 410 `rejected/replayed`**). Deactivation notice **display** confirmed on both (DOM + screenshot, with the conditional support link).
- **Release-blocking WSOD found+fixed:** `box-packer/class-item-implementation.php` implements `Woodev_Box_Packer_Item_With_Product` but the interface file was never required in `includes()` → every real vendored v2 boot fataled (classmap masked it in tests). First live boot caught it. One-line fix.
- **Shipped 2.0.1 (branch `feat/s11-licensing-2.0.1`, PR #41):** neutral remote-deactivation notice + conditional support link (+ tests); `woodev_licensing_api_url` filter; the interface fix; VERSION 2.0.0→2.0.1. `composer check` green (PHPCS/PHPStan 0/603 unit). Self-merge on green CI.
- **Findings:** option-key double-prefix for `woodev`-prefixed ids (real plugins unaffected; gotcha); B-1 mixed-fleet armor confirmed live (v1+v2 can't be co-active until migration — no WSOD); `wp_safe_remote_request` blocks private host+port (local-rig filters in the stand only).
- **Reusable local rig left running:** issuer woodev_theme wp-env :8090, stand framework wp-env :8888 (gitignored `.wp-env-stand/`). woodev_theme has one local-only change: `Push_Delivery::is_safe_target()` returns true under `env=local` (prod unaffected). For the edostavka migration testing.
