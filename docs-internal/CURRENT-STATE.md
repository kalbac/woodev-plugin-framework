...
- 🎯 **s26 part 2 (queued) — #8 install-from-connector:** `WP_Upgrader` + connector `/download/{id}` (EDD SL package URL). **Security-critical → own focused session, mandatory Codex adversarial-review.** «Куплено» badge already gives the entry point. Rating-in-API: dropped (operator). Bootstrap-instant-badges: dropped (operator — non-critical).
