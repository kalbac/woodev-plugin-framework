...
1. **Проверить, что Serena поднялась** (фикс `MCP_TIMEOUT` применён в s61, нужен был только рестарт). Обязательно до #244.
2. **#238 — доделать фикс: токен УДАЛИТЬ.** Ветка `feat/238-cart-change-invalidation` несёт два подтверждённых HIGH и коммит `wip(...) DO NOT MERGE`. Читать существующее состояние `refreshWaiter`/`refreshTimer` в момент события вместо `echoExpected`. Два регрессионных теста обязательны и должны падать на текущем коде. Полный разбор → `next-session-prompt.md`.
3. **#243 — ждёт ОПЕРАТОРА**, не агента. PR #246, CI зелёный. Риг переключить на её ветку (он отдаёт рабочее дерево).
4. **#214 — ждёт решения ОПЕРАТОРА.** Замер сделан: B+ = 93% исправлений, 0 сломанных корректных адресов.
5. **#245** (release-blocking, нужен прод-ключ от оператора — агент его не выдумает), **#244** (P4-извлечения, нужна Serena), **#247** (минификация, Инбокс — карточка оператора), **#248** (viewport `lastBbox`, Бэклог).
