...
`composer check` green at s29: **780 unit tests** / 2237 assertions (66 skipped), integration green (incl. new `SetupWizardRestTest`). Keep green after each change. Note: PHPStan crashes locally on Windows (`-1073741819`, environmental — gotcha `phpstan-windows-parallel-worker-segfault`); Linux CI "Run PHPStan" is the authoritative gate.
