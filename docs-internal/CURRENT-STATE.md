...
**As of 2026-08-16 (after s74).** `main` = `6c030da` · no open PRs · working tree clean, rig on `main`.
Tests: **2272 unit / 5632 assertions / 1117 jest / 110 integration**; phpcs and phpstan clean.
Gotchas: **151**.
...
0. **🎯 Доска — свободных «взять и делать» карточек три.**
   - **#339** — выбор ПВЗ переписывает поле НП, каскад законно сбрасывает запись. Оператор ответил 16.08.2026: «ничего не делать» отвергнуто (при стратегии `viewport` покупатель легко уходит за скоуп города). Его предложение — прогонять адрес ПВЗ через `Location_Provider::normalize()` и перезаписывать поля обогащёнными данными; просил проверить до принятия. Шов существует, DaData реализует его через платный Clean API (отдельный секрет `clean_secret`), на риге оба ключа настроены — замер возможен. Замер обязан установить точность, деградацию (`normalize()` отдаёт `null` без секрета и на сетевой ошибке; провайдер не обязан его реализовывать) и цену (платный вызов на КАЖДЫЙ выбор пункта).
...
2. **Отложено оператором до релиза:** #285, #247. **Старое:** #289, #270, #310, #318, #321, #322.
3. **Тулинг:** Codex чинится одной командой из-под администратора — `winget install --id Microsoft.PowerShell`. Готча `codex-shell-sandbox-broken-windows`.
