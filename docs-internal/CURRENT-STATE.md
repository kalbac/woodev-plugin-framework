...
> Last updated: 2026-06-13 (session 11: LIVE remote-deactivation e2e proven on BOTH channels — push (prod woodev.ru + public staging) and pull (local two-stack rig) — plus replay-reject (HTTP 410). Found+fixed a release-blocking WSOD (box-packer interface unwired). Framework **2.0.1** on PR #41 (notice rewrite + api_url filter + interface fix + bump), `composer check` green 603 tests. **Next:** edostavka pilot — START with shipping-module audit (operator's manual-review list), then migration spec + data-preservation contract test.)
