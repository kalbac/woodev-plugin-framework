...
> Last updated: 2026-06-22 (session 29 — Setup Wizard (OB-10) rewrite SHIPPED (PR #80 `ce04700`); 780 unit; autonomous overnight; final independent review deferred — see s29 log).
...
`composer check` green at s29: **782 unit tests** / 2239 assertions (66 skipped), integration green (incl. new `SetupWizardRestTest` + `RestV1RegistrarKeyTest`). Keep green after each change. Note: PHPStan crashes locally on Windows (`-1073741819`, environmental — gotcha `phpstan-windows-parallel-worker-segfault`); Linux CI "Run PHPStan" is the authoritative gate.
