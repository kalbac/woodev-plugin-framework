...
- [ymaps-camera-moves-are-async](ymaps-camera-moves-are-async.md) — the original problem `focusGroup()`/`_focusSeq` solve
- [ymaps-html-icon-layout-needs-iconshape](ymaps-html-icon-layout-needs-iconshape.md) — same session, same "click behaviour only verified by
