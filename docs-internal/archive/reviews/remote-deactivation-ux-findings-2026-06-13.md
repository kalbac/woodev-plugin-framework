> Archived s60 (2026-08-09): findings all triaged/closed (B-13/14/15 resolved s10-s12).

# Remote-deactivation UX findings (operator manual run, s11, 2026-06-13)

> **RESOLVED in s12 (2026-06-13).** A — `handle_activation()` clears the stale notice option entry on reactivation (kept). B — a WC Admin Notes breadcrumb was tried (PR #44) then **REVERTED per operator (PR #46)**: a single-v2-plugin site intentionally shows no banner (the kill-switch targets violators — nothing is lost), and two surfaces on a multi-plugin site is redundant. Final behavior = banner-only, shown only when ≥2 active v2 plugins. C — was a pull-only rig artifact; real fix = «Отменить»(queued) vs «Снять с доставки»(delivered) wording, and re-deactivation-after-terminal verified. Framework PR #44 (`21bb436`) + PR #46 (B revert); deactivator local `28af8b9`. Rig-verified vs real WC 10.8.1. See SESSION-LOG s12.
