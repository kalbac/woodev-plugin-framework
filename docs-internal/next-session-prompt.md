...
## Task0 — подчистить старые ветки (оператор-реквест, в начале s40)

Оператор не любит, когда копится мусор. В репо висят 4 старые локальные ветки НЕ из текущей работы. Статус проверен 01.07.2026 — действовать БЕЗОПАСНО (не force-удалять несмерженное вслепую):

- ✅ **`worktree-agent-a113f54577018d3fd`** — MERGED в main (PR #57), worktree не привязан → удалить сразу: `git branch -d worktree-agent-a113f54577018d3fd`.
- ⚠️ **`feat/platform-v2-epic1-spike`** (спайк, 28.05), **`feat/s3-licensing-need-license`** (s5, 10.06), **`polish/settings-page-ui`** (s35; ЕСТЬ на origin: `origin/polish/settings-page-ui`; в истории помечена «kept as reference») — все три **NOT-merged** (их тип-коммиты не в main), но фичи зашипены ИНАЧЕ (S3 licensing через PR #25/#31/#35; settings UI → UI-kit + SP-3). Почти наверняка суперсед/мусор. **Подтвердить у оператора одной фразой**, затем `git branch -D <branch>` (force, т.к. NOT-merged); для polish ещё `git push origin --delete polish/settings-page-ui`.

После — `git branch` должен показывать только `main`.
